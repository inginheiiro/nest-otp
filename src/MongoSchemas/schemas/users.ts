import {model, Schema} from 'mongoose';
import {RolesEnum} from '../rolesEnum';


export const UsersSchema = new Schema({
    name: {type: String, required: false},
    phone: {type: String, required: false},
    photo: {
        type: String,
        required: false,
        default:
            'data:image/jpeg;base64,'
    },
    email: {type: String, required: true},
    deleted: {type: Boolean, required: true, default: false},
    secretOTP: {type: String, required: true},
    roles: {
        type: [String],
        enum: [
            RolesEnum.ADMIN,
            RolesEnum.TEACHER,
            RolesEnum.PARENT,
            RolesEnum.STUDENT,
            RolesEnum.NOROLE
        ],
        default: [RolesEnum.NOROLE]
    }
});

UsersSchema.pre('save', function (next) {
    const self = this;

    if (!self.isModified('roles')) {
        return next();
    }
    if (self.noRoles() && self.roles.length > 1) {
        self.roles = delete self.roles[RolesEnum.NOROLE];
        next();
    }
});

UsersSchema.methods.isAdmin = function isAdmin() {
    return this.roles.find(p => p === RolesEnum.ADMIN);
};

UsersSchema.methods.noRoles = function noRoles() {
    return this.roles.find(p => p === RolesEnum.NOROLE);
};

UsersSchema.methods.isParent = function isParent() {
    return this.roles.find(p => p === RolesEnum.PARENT);
};

UsersSchema.methods.isStudent = function isStudent() {
    return this.roles.find(p => p === RolesEnum.STUDENT);
};

UsersSchema.methods.isTeacher = function isTeacher() {
    return this.roles.find(p => p === RolesEnum.TEACHER);
};


export const UsersModel = model(
    'Users',
    UsersSchema
);
